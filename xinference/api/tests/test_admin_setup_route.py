...
Covers the Gemini review finding on PR #5144: once setup has completed,
``setup_admin`` must reject immediately -- before validating the password
or computing its bcrypt hash -- so the endpoint doesn't stay a permanently
unauthenticated, CPU-expensive route after the first admin exists.
...
def test_setup_succeeds_when_no_user_exists(auth_service):
