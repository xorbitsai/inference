...
class ProfilerHook(Hook):
    ...
    def __init__(
        ...
        if profile_iters < 1:
            raise ValueError(
                "profile_iters should be greater than 0, but got " f"{profile_iters}"
            )
    ...
    def before_run(self, runner):
        if self.by_epoch and runner.max_epochs < self.profile_iters:
            raise ValueError(
                "self.profile_iters should not be greater than " f"{runner.max_epochs}"
            )

        if not self.by_epoch and runner.max_iters < self.profile_iters:
            raise ValueError(
                "self.profile_iters should not be greater than " f"{runner.max_iters}"
            )
