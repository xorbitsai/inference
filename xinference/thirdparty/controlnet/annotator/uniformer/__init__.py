...
def apply_uniformer(img):
    ...
    if model is None:
        ...
        model = init_segmentor(
            config_file, modelpath, device=torch.device(get_available_device())
        )
    model = model.to(torch.device(get_available_device()))

    if torch.device(get_available_device()).type == "mps":
