...
from ..util import nms, safe_step
...
def apply_hed(input_image, is_safe=False):
    ...
    if netNetwork is None:
        ...
        if os.path.exists(old_modelpath):
            ...
            hf_hub_download("lllyasviel/Annotators", "ControlNetHED.pth",
                            local_dir=modeldir)
        netNetwork = ControlNetHED_Apache2().to(get_available_device())
        netNetwork.load_state_dict(torch.load(modelpath, map_location="cpu"))
    netNetwork.to(get_available_device()).float().eval()
