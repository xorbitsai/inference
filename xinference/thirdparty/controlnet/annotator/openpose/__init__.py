...
class OpenposeDetector:
    ...
    def load_model(self):
        ...
        if not os.path.exists(hand_modelpath):
            hf_hub_download("lllyasviel/Annotators", "hand_pose_model.pth",
                            local_dir=self.model_dir)

        if not os.path.exists(face_modelpath):
            hf_hub_download("lllyasviel/Annotators", "facenet.pth",
                            local_dir=self.model_dir)
    ...
    def load_dw_model(self):
        ...
        def load_model(filename: str, repo_id: str):
            local_path = os.path.join(self.model_dir, filename)
            if not os.path.exists(local_path):
                hf_hub_download(repo_id, filename, local_dir=self.model_dir)

            return local_path

        onnx_det = load_model("yolox_l.onnx", "yzd-v/DWPose")
        onnx_pose = load_model("dw-ll_ucoco_384.onnx", "yzd-v/DWPose")
    ...
    def load_animalpose_model(self):
        ...
        def load_model(filename: str, repo_id: str, path: str = None):
            """
            ...
            Args:
                ...
                repo_id (str): The repo ID of the model.
            """
            local_path = os.path.join(self.model_dir, filename)
            if not os.path.exists(local_path):
                if path is None:
                    path = filename
                hf_hub_download(repo_id, path, local_dir=self.model_dir)
                if path != filename:
                    shutil.move(os.path.join(self.model_dir, path),
                                local_path)
            return local_path
        ...
        onnx_pose = load_model(
            ...
            path="Annotators/rtmpose-m_simcc-ap10k_pt-aic-coco_210e-256x256-7a041aa1_20230206.onnx"
        )
