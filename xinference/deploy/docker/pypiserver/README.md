...
preloaded with every wheel the xinference runtime may install into per-model
virtual environments, for offline / air-gapped deployments. It is the default
package source of the `offline` compose profile in `xinference/deploy/docker`
(see the "Offline / Air-gapped Deployment" section of the Docker Compose docs).

The GPU stack targets CUDA 13.0 and Python 3.12 (matching the
`xprobe/xinference` runtime image). CUDA versions below 13.0 are not supported.
...
2. **`download_packages.py`** locks the torch family to the runtime image's
   ...
   wheels.
3. **`selfcheck.py`** (a dedicated build stage) re-resolves every engine set,
   pin and direct wheel against the baked mirror ALONE (`--disable-fallback`).
   Any gap fails the image build instead of failing an air-gapped user at
   model-launch time.
