        ...
        **kwargs
        ...
        if isinstance(image, list):
            ...
            for i, img in enumerate(image):
                if isinstance(img, str):
                    ...
                    with open(img, 'rb') as f:
                        content = f.read()
                    data.add_field(f"image[]", content, filename=f"image_{i}.png", content_type="image/png")
                else:
                    # Binary data
                    data.add_field(f"image[]", img, filename=f"image_{i}.png", content_type="image/png")
        else:
            # Single image
            if isinstance(image, str):
                # File path - read file content
                with open(image, 'rb') as f:
                    content = f.read()
                data.add_field("image", content, filename="image.png", content_type="image/png")
            else:
                # Binary data
        ...
        if mask is not None:
            if isinstance(mask, str):
                ...
                with open(mask, 'rb') as f:
                    content = f.read()
                data.add_field("mask", content, filename="mask.png", content_type="image/png")
            else:
                # Binary data
