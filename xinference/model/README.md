...
`models/index.json` lists `[filename, record_index]` pairs in load order. Keep
existing entries in place when updating a model. When adding or removing a
record, update the index too. Separate references preserve the previous order
even when variants of one model were interleaved with other models.
...
read only the first record of a model file or sort records by filename.
...
The Hub's repository PR writer must target the new model files and index. For a
...
update, preserve the other model files and index entries.
...
documentation workflow accepts only catalog JSON data, validates the index and
...
includes the model files and their index recursively.
