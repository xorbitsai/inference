...
import type { TokenRouterItem } from '@/types/services';
...
export default function TokenRouterPage() {
  ...
  const load = useCallback(async (mode: 'initial' | 'manual' | 'silent' = 'silent') => {
    ...
    try {
      const data = await request.get<TokenRouterItem[]>('/v1/token_routers');
  ...
  useEffect(() => {
    void load('initial');
    const timer = window.setInterval(() => void load('silent'), 10000);
    ...
    <PageContainer
      ...
      extraContent={
        <div className="flex flex-wrap justify-end gap-2">
          <Button variant="outline" loading={refreshing} onClick={() => void load('manual')}>
              ...
              routers.map((router) => {
                ...
                return (
                  <TableRow key={router.router_uid}>
                    ...
                    <TableCell className="whitespace-nowrap">{router.revision}</TableCell>
