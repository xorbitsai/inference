const zh = {
  ...
  launchModel: {
    ...
    numSpeculativeTokensTip:
      '每轮草稿模型提议的 token 数（含 bonus token）。只有 MLX 会从草稿模型自身读取这个深度（Gemma 4 是 4）；其他引擎必须给值，这里用的是各自 Gemma 4 官方指南里的值：vLLM 1、SGLang 6、llama.cpp 3。在 MLX 上填更大的值只作为上限 —— 每轮仍从训练深度起步，只有接受率足够高时才会增长；其他引擎按填的值执行，其中 vLLM 的 1 偏保守，往上调通常有收益。',
