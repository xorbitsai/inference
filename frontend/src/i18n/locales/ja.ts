const ja = {
  ...
  launchModel: {
    ...
    numSpeculativeTokensTip:
      '1 ラウンドあたりにドラフトモデルが提案するトークン数（ボーナストークンを含む）。この深度をドラフトモデルから読み取るのは MLX のみ（Gemma 4 は 4）。他のエンジンは値が必須なため、各エンジンの Gemma 4 ガイドの値を使用します: vLLM は 1、SGLang は 6、llama.cpp は 3。MLX では大きい値は上限として扱われ、各ラウンドは学習深度から始まり受理履歴が良好な場合にのみ増加します。他のエンジンは指定値をそのまま使い、特に vLLM の 1 は保守的です。',
