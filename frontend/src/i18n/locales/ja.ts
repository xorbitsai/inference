const ja = {
  ...
  launchModel: {
    ...
    'requestLimits.optional':
      '(オプション) リクエスト制限、このモデルのリクエスト制限数、デフォルトは無制限',
    'workerIp.optional':
      '(オプション) ワーカーIP、分散シナリオでモデルが配置されているワーカーIPを指定,カンマで区切ることで複数の IP を指定できます。',
    ...
    'modelPath.optional':
      '(オプション) モデルパス、PyTorchの場合はモデルディレクトリ、GGML/GGUFの場合はモデルファイルパスを指定',
