const ko = {
  ...
  launchModel: {
    ...
    numSpeculativeTokensTip:
      '라운드당 드래프트 모델이 제안하는 토큰 수(보너스 토큰 포함). 이 깊이를 드래프트 모델에서 읽는 것은 MLX뿐입니다(Gemma 4는 4). 다른 엔진은 값이 필요하므로 각 엔진의 Gemma 4 가이드 값을 사용합니다: vLLM 1, SGLang 6, llama.cpp 3. MLX에서는 더 큰 값이 상한으로 동작하여 각 라운드가 학습 깊이에서 시작하고 수용 이력이 충분할 때만 늘어나며, 다른 엔진은 지정한 값을 그대로 사용합니다. 특히 vLLM의 1은 보수적입니다.',
