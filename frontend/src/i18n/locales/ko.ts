const ko = {
  ...
  launchModel: {
    ...
    numSpeculativeTokensTip:
      '라운드당 드래프트 모델이 제안하는 토큰 수(보너스 토큰 포함). 비워 두면 세 가지로 나뉩니다: MLX는 드래프트 모델에서 학습 깊이를 읽고(Gemma 4는 4), llama.cpp는 xllamacpp 자체 기본값(3)을 사용하며, vLLM과 SGLang은 값이 필요해 각 Gemma 4 가이드의 값(1과 6)을 전달합니다. MLX에서는 더 큰 값이 상한으로 동작하여 각 라운드가 학습 깊이에서 시작하고 수용 이력이 충분할 때만 늘어나며, 다른 엔진은 지정한 값을 그대로 사용합니다. 특히 vLLM의 1은 보수적입니다.',
