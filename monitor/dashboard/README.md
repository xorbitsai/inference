...
Pre-built Grafana dashboards for monitoring Xinference clusters. The original monolithic dashboard has been split into 6 focused sub-dashboards, each targeting a specific audience and use case.
...
| File | Description |
|------|-------------|
| `generate.py` | Python script to regenerate all 24 dashboard JSONs from the original monolithic dashboard |
...
The 24 dashboard JSON files are generated by `generate.py`. To regenerate:
...
The script reads the original monolithic dashboard JSON and redistributes panels into 6 sub-dashboards with 4-language translations. UID consistency across languages is verified automatically.
