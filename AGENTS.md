...
New or changed English documentation must include the corresponding Simplified
Chinese gettext updates under `doc/source/locale/zh_CN/LC_MESSAGES/`. Keep PO
changes scoped to the affected messages, validate the modified catalogs with
`msgfmt --check --check-format`, compile them with
`python doc/build_i18n.py zh_CN`, and build the changed page with
`language=zh_CN` to verify the rendered translation.
