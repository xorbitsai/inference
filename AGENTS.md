...
New or changed English documentation must include corresponding gettext updates
for every existing locale under `doc/source/locale/*/LC_MESSAGES/`. Keep PO
changes scoped to the affected messages, validate every modified catalog with
`msgfmt --check --check-format`, compile them with
`python doc/build_i18n.py --all`, and build the changed page in each maintained
language to verify the rendered translations.
...
The PR workflow in `.github/workflows/pr_auto_run_gen_docs.yaml` only pushes
