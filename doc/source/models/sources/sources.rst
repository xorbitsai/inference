...
configured via ``HTTP_PROXY``, ``HTTPS_PROXY``, or ``ALL_PROXY``, automatic detection
selects ModelScope without probing so that large model downloads do not consume proxy
traffic. ``NO_PROXY`` is honored, so an endpoint excluded from the proxy is still probed
directly and selects Hugging Face when reachable. An HTTP error response counts as
